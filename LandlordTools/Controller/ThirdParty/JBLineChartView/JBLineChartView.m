...
//                NSAssert(isnan(rawHeight) || (rawHeight >= 0), @"JBLineChartView // delegate function - (CGFloat)lineChartView:(JBLineChartView *)lineChartView verticalValueForHorizontalIndex:(NSUInteger)horizontalIndex atLineIndex:(NSUInteger)lineIndex must return a CGFloat >= 0 OR NAN");
                ...
                if (!_labelCreated) {
                    ...
                    if ([self.delegate respondsToSelector:@selector(titleForBand:withIndex:)]) {
                        UILabel* ratesLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 60, 20)];
                        ratesLabel.textAlignment = NSTextAlignmentCenter;
                        ratesLabel.text = [self.dataSource titleForBand:self withIndex:horizontalIndex];
                        ratesLabel.textColor = [UIColor whiteColor];
                        ratesLabel.font = [UIFont systemFontOfSize:14];
                        ratesLabel.backgroundColor = [UIColor clearColor];
                        ratesLabel.center = CGPointMake(xOffset, self.bounds.size.height - 5);
                        [self addSubview:ratesLabel];
                    }
                    
                    if (horizontalIndex == 0) {
                        if ([self.dataSource respondsToSelector:@selector(middleBandLevelString:)]
                            && [self.dataSource respondsToSelector:@selector(maxBandLevelString:)]
                            && [self.dataSource respondsToSelector:@selector(minBandLevelString:)]) {
                            
                            NSInteger tempBandLevel = 0;
                            CGFloat tempYPosition = 0;
                            NSInteger minBandLevel = [self.dataSource minBandLevelString:self];
                            ...
                            tempBandLevel = middleBandLevel;
                            tempYPosition = self.center.y;
                            while (tempBandLevel <= minBandLevel) {
                                
                                UILabel *bandLevel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
                                bandLevel.text = [NSString stringWithFormat:@"%ld", (long)tempBandLevel];
                                bandLevel.textAlignment = NSTextAlignmentCenter;
                                bandLevel.backgroundColor = [UIColor clearColor];
                                bandLevel.textColor = [UIColor whiteColor];
                                bandLevel.font = [UIFont systemFontOfSize:14];
                                bandLevel.center = CGPointMake(xOffset - 20.0f, tempYPosition);
                                [self addSubview:bandLevel];
                                
                                tempYPosition += yOffsetSpace;
                                tempBandLevel += 10;
                            }
                            
                            UILabel *nagtiveSign = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
                            nagtiveSign.text = @"-";
                            nagtiveSign.textAlignment = NSTextAlignmentCenter;
                            nagtiveSign.backgroundColor = [UIColor clearColor];
                            nagtiveSign.textColor = [UIColor whiteColor];
                            nagtiveSign.font = [UIFont systemFontOfSize:14];
                            nagtiveSign.center = CGPointMake(xOffset - 20.0f, tempYPosition);
                            [self addSubview:nagtiveSign];
                            
                            tempBandLevel = middleBandLevel;
                            tempYPosition = self.center.y;
                            while (tempBandLevel <= maxBandLevel) {
                            ...
                            UILabel *positiveSign = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
                            positiveSign.text = @"+";
                            positiveSign.textAlignment = NSTextAlignmentCenter;
                            positiveSign.backgroundColor = [UIColor clearColor];
                            positiveSign.textColor = [UIColor whiteColor];
                            positiveSign.font = [UIFont systemFontOfSize:14];
                            positiveSign.center = CGPointMake(xOffset - 20.0f, tempYPosition);
                            [self addSubview:positiveSign];
                        }
...
- (CGFloat)minimumValue
{
    ...
    {
        NSLog(@"The cache min height : %f", self.cachedMaxHeight);
...
- (CGFloat)maximumValue
{
    ...
    {
        NSLog(@"The cache max height : %f", self.cachedMaxHeight);
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (NSArray *lineData in chartData)
    {
        ...
        BOOL smoothLine = YES; // [self.delegate lineChartLinesView:self smoothLineAtLineIndex:lineIndex];
        ...
        for (NSUInteger index = 0; index < [sortedLineData count]; index++)
        {
            ...
            {
                [path moveToPoint:CGPointMake(lineChartPoint.position.x, self.center.y)];
            ...
            currentXPosition = lineChartPoint.position.x;
            
//            if ([self.delegate respondsToSelector:@selector(titleForBand:withIndex:)]) {
//                UILabel* ratesLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 40, 20)];
//                ratesLabel.textAlignment = NSTextAlignmentCenter;
//                ratesLabel.text = [self.delegate titleForBand:self withIndex:index];
//                ratesLabel.backgroundColor = [UIColor clearColor];
//                ratesLabel.center = CGPointMake(currentXPosition, self.bounds.size.height - 20);
//                [self addSubview:ratesLabel];
//            }
        }
        
        [path addLineToPoint:CGPointMake(lastXPosition, self.center.y)];
