...
- (CGRect)trackRect
{
    ...
    if(currentTrackImage.capInsets.top || currentTrackImage.capInsets.bottom || currentTrackImage.capInsets.left || currentTrackImage.capInsets.right)
    {
        retValue = UIEdgeInsetsInsetRect(retValue, currentTrackImage.capInsets);
    }
    
    if (_maximumValue == 0) {
        retValue.origin = CGPointMake(0, (self.bounds.size.height/2.0f) - (retValue.size.height/2.0f));
...
- (CGRect)thumbRectForValue:(float)value image:(UIImage*) thumbImage
{
    ...
    {
        thumbRect = UIEdgeInsetsInsetRect(thumbRect, insets);
    }
...
-(void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if ([self.delegate respondsToSelector:@selector(sliderValueChanged:lowwerValue:upperValue:)]) {
        [self.delegate sliderValueChanged:self lowwerValue:_lowerValue upperValue:_upperValue];
    }
